...
| `EMIT_L1B_RAD_002_20260921T044051` | `EMITL2AMASK.003/.../EMIT_L2A_MASK_003_20260921T044051.nc` | none (`get_ch4enhancement_link` returns `None`) |
...
The reader lives in the `georeader.readers.emit` package: `utils` (product names and versions), `download` (Earthdata download and DAAC links), `mask` (L2A mask flags) and `image` (`EMITImage`). Everything is re-exported from `georeader.readers.emit`, so `from georeader.readers.emit import EMITImage` and `emit.TOKEN = ...` work as before.

### API Reference

::: georeader.readers.emit.image
    options:
      members:
        - EMITImage

::: georeader.readers.emit.utils
    options:
      members:
        - EMITProductID
        - parse_product_name
...
::: georeader.readers.emit.download
    options:
      members:
        - download_product
        - get_radiance_link
